...
template <typename Float, typename F>
Eigen::VectorX<Float> apply_function_uniform(Float x_min, Float x_max, uint32_t nx,
                                           Float y_min, Float y_max, uint32_t ny,
                                           Float z_min, Float z_max, uint32_t nz, F f) {
...
int main() {
  ...
  const f_ty dx = 2 * Lx / (nx - 1), dy = 2 * Ly / (ny - 1), dz = 2 * Lz / (nz - 1);
  ...
  auto spatial_soliton = [&](c_ty x, c_ty y, c_ty z) {
    ...
    std::complex<f_ty> phase(0.0, kx * std::real(x) + ky * std::real(y) + kz * std::real(z));
    return A * norm * std::exp(-(x*x + y*y + z*z)/(2.0*sigma*sigma)) *
           std::exp(phase) *
  ...
  Eigen::VectorX<c_ty> u0 = apply_function_uniform<c_ty>(-Lx, Lx, nx, -Ly, Ly, ny, 
                                                        -Lz, Lz, nz, f_vortex_ring);
  ...
  Eigen::Map<Eigen::Matrix<c_ty, -1, -1, Eigen::RowMajor>>(
      u_save.data(), num_snapshots, nx * ny * nz).row(0) = u0.transpose();
  ...
  for (uint32_t i = 1; i < nt; ++i) {
    ...
    if (i % freq == 0) {
      Eigen::Map<Eigen::Matrix<c_ty, -1, -1, Eigen::RowMajor>>(
          u_save.data(), num_snapshots, nx * ny * nz).row(i / freq) = u.transpose();
    }
    PROGRESS_BAR(i, nt);
  }
