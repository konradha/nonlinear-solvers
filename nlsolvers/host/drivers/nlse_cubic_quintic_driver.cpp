...
int main(int argc, char** argv) {
    if (argc != 12 && argc != 13) {
        std::cerr << "Usage: " << argv[0]
                  << " nx ny Lx Ly s1 s2 input_u0.npy output_traj.npy T nt num_snapshots [input_m.npy]\n";
        std::cerr << "Example: " << argv[0]
                  << " 256 256 10.0 10.0 0.6 -0.6 initial.npy evolution.npy 1.5 500 100\n";
        std::cerr << "Example with m(x,y): " << argv[0]
                  << " 256 256 10.0 10.0 0.6 -0.6 initial.npy evolution.npy 1.5 500 100 focusing.npy\n";
        return 1;
    }
    ...
    for (uint32_t i = 1; i < nt; ++i) {
        NLSEQuinticSolver::step<std::complex<double>>(buf, rho_buf, u, L, m, dti, s1, s2); 
        if (i % freq == 0) {
