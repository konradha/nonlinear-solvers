...
void step(Eigen::VectorX<Scalar_t> &buf, Eigen::VectorX<Scalar_t> &rho_buf,
  ...
  // u^{n+1} = e^{-2i*dt*Delta}u^{n-1} +
  2*dt*e^{-i*dt*Delta}*phi_s(dt*Delta)*B(u^n) u = exp_u_prev + 2. * tau *
  exp_phi_s_B_un; u_prev = rho_buf;
  */
  ...
  auto compute_B = [&m](const Eigen::VectorX<Scalar_t> & u) {
    auto u_abs_squared = u.real().cwiseProduct(u.real()) +
                        u.imag().cwiseProduct(u.imag());
    return -(m.cwiseProduct(u_abs_squared)).cwiseProduct(u); // sign unclear
  from paper
  };
  buf = compute_B(u);
  ...
  2i*tau*e^{-i*tau*Delta}*phi_s(tau*Delta)*B(u^n) u = exp_u_prev - 2. * tau *
  exp_phi_s_B_un; u_prev = rho_buf;
  */
}
...
template <typename Scalar_t>
Eigen::VectorX<Scalar_t>
phi1m_multiply(const Eigen::SparseLU<Eigen::SparseMatrix<Scalar_t>> &solver,
               const Eigen::SparseMatrix<Scalar_t> &L,
               const Eigen::VectorX<Scalar_t> &v, const Scalar_t tau) {
