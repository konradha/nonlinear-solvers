...
void experimental_nlse_envelope_bc(Eigen::VectorX<std::complex<double>>& u, uint32_t nx, uint32_t ny, double dx, double dy,
        const Eigen::VectorX<std::complex<double>> & m) {

    Eigen::VectorX<std::complex<double>> u_copy = u;
    
    auto idx = [nx, ny](uint32_t i, uint32_t j) {
        return i * (ny + 2) + j;
    };
    
    auto compute_effective_k = [&](uint32_t i, uint32_t j, double h) {
        std::complex<double> laplacian = (u_copy[idx(i+1, j)] + u_copy[idx(i-1, j)] + 
                                     u_copy[idx(i, j+1)] + u_copy[idx(i, j-1)] - 
                                     4.0 * u_copy[idx(i, j)]) / (h * h);
        
        std::complex<double> nonlinear_term = m[idx(i, j)] * std::norm(u_copy[idx(i, j)]);
        std::complex<double> k_squared = -laplacian / u_copy[idx(i, j)] + nonlinear_term;
    ...
    for (uint32_t j = 1; j <= ny; ++j) {
        ...
        std::complex<double> phase_left = std::exp(std::complex<double>(0, -k_left * dx));
        u[idx(0, j)] = phase_left * u_copy[idx(1, j)];
        
        double k_right = compute_effective_k(nx, j, dx);
        std::complex<double> phase_right = std::exp(std::complex<double>(0, -k_right * dx));
        u[idx(nx+1, j)] = phase_right * u_copy[idx(nx, j)];
    }
    
    for (uint32_t i = 1; i <= nx; ++i) {
        double k_bottom = compute_effective_k(i, 1, dy);
        std::complex<double> phase_bottom = std::exp(std::complex<double>(0, -k_bottom * dy));
        u[idx(i, 0)] = phase_bottom * u_copy[idx(i, 1)];
        
        double k_top = compute_effective_k(i, ny, dy);
        std::complex<double> phase_top = std::exp(std::complex<double>(0, -k_top * dy));
