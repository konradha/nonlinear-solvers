...
__global__ void neg_sin_kernel(double *out, const double *in, const uint32_t n) {
...
__global__ void gautschi_kernel(double *u_next, const double *u, const double *u_past, 
...
__global__ void stormer_verlet_kernel(double *u_next, const double *u, const double *u_past, 
                                    const double *lapl_sin_term, const double tau, 
                                    const uint32_t n) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
__global__ void compute_lapl_sin_kernel(double *out, const double *Lu, const double *sin_term, 
                                      const uint32_t n) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
void step_sv(double *d_u, double *d_u_past, double *d_buf, double *d_buf2,
           DeviceSpMV<double> *spmv, const double *d_m,
           const double tau, const uint32_t n, const dim3 grid, const dim3 block) {
...
void step(double *d_u, double *d_u_past, double *d_buf, double *d_buf2, 
  ...
  matfunc->apply(d_buf2, d_u, tau, MatrixFunctionApplicatorReal::FunctionType::ID_SQRT);
  neg_sin_kernel<<<grid, block>>>(d_buf2, d_buf2, n);
  matfunc->apply(d_buf3, d_buf2, tau, MatrixFunctionApplicatorReal::FunctionType::SINC2_SQRT);
