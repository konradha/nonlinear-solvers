#!/bin/bash

#SBATCH --job-name=kge_2d_dev
#SBATCH --time=00:40:00
#SBATCH --mem-per-cpu=32G
#SBATCH --gpus=1
#SBATCH --output=logs/kge2d_solver_dev_%j.out
#SBATCH --error=logs/kge2d_solver_dev_%j.err
#SBATCH --ntasks=1
#SBATCH --partition=gpu

module load stack/2024-06 cuda/12.1.1 python/3.11.6 eigen
module load python_cuda/3.11.6
module load gcc/12.2.0 
module load ffmpeg

n=220
nt_breather=15000 # using SV we're really _fast_
nt_other=6000
dr=128
exe=build/bin/kg_sv_2d_dev
L=3.
T_breather=20.
T_other=5.
snap=100

# constant: u_tt = \Delta u - u
for p in kink_solution kink_field kink_array_field multi_breather_field breather_solution breather_field; do
	for v in zero random; do
        	python scripts_sge_kge/real_launcher.py \
        	        --system-type klein_gordon --phenomenon ${p} --velocity-type ${v} \
        	        --nx $n --ny $n --Lx $L --Ly $L --nt ${nt_breather} --T ${T_breather} --snapshots ${snap} \
        	        --dr-x ${dr} --dr-y ${dr} --delete-intermediates \
        	        --exe ${exe} --num-runs 4 \
        	        --output-dir=$SCRATCH/kge_2d_${p}_${v}
	done
done

for p in spiral_wave_field multi_spiral_state ring_soliton colliding_rings \
        multi_ring_state skyrmion_solution skyrmion_lattice skyrmion_like_field q_ball_solution \
        multi_q_ball elliptical_soliton grf_modulated_soliton_field; do
	for v in zero random; do	
        	python scripts_sge_kge/real_launcher.py \
                	--system-type klein_gordon --phenomenon ${p} --velocity-type ${v} \
                	--nx $n --ny $n --Lx $L --Ly $L --nt ${nt_other} --T ${T_other} --snapshots ${snap} \
                	--dr-x ${dr} --dr-y ${dr} --delete-intermediates \
                	--exe ${exe} --num-runs 4 \
                	--output-dir=$SCRATCH/kge_2d_${p}_${v}
	done
done

# for p in kink_field; do
# 	python scripts_sge_kge/real_3d_launcher.py \
#                 --exe ${exe} \
#                 --nx $n --ny $n --nz $n \
#                 --dr-x ${dr} --dr-y ${dr} --dr-z ${dr} \
#                 --Lx $L --Ly $L --Lz $L \
#                 --T 8. --nt ${nt} --snapshots 40 \
#                 --num-runs 2 \
# 		--phenomenon ${p} \
# 		--visualize \
#                 --seed $((SLURM_JOB_ID + SLURM_ARRAY_TASK_ID)) \
#                 --output-dir /cluster/scratch/konradha/kge_${p}_constant
# done


## non-constant: u_tt = div(c(x,y,z)grad(u)) - m(x,y,z) u
#for p in kink_field; do
#	for cp in optimal sharp_interfaces waveguide grf_threshold anisotropic; do
#		python scripts_sge_kge/real_3d_launcher.py \
#			--exe ${exe} \
#			--nx $n --ny $n --nz $n \
#			--dr-x ${dr} --dr-y ${dr} --dr-z ${dr} \
#			--Lx $L --Ly $L --Lz $L \
#			--T 8. --nt ${nt} --snapshots 40 \
#			--num-runs 8 \
#			--phenomenon ${p} \
#			--c-m-pair ${cp} \
#			--seed $((SLURM_JOB_ID + SLURM_ARRAY_TASK_ID)) \
#			--output-dir /cluster/scratch/konradha/kge_${p}_${cp}
#	done
#done

# for p in kink_field; do
# 	for cp in optimal sharp_interfaces waveguide grf_threshold anisotropic; do
# 		python scripts_sge_kge/real_3d_launcher.py \
# 			--exe ${exe} \
# 			--nx $n --ny $n --nz $n \
# 			--dr-x ${dr} --dr-y ${dr} --dr-z ${dr} \
# 			--Lx $L --Ly $L --Lz $L \
# 			--T 8. --nt ${nt} --snapshots 40 \
# 			--num-runs 2 \
# 			--visualize \
# 			--phenomenon ${p} \
# 			--c-m-pair ${cp} \
# 			--seed $((SLURM_JOB_ID + SLURM_ARRAY_TASK_ID)) \
# 			--output-dir /cluster/scratch/konradha/kge_${p}_${cp}
# 	done
# done
