...
class LanczosStepTorch(torch.nn.Module):
    ...
    def forward(self, buf1, V, T, j, m):
        ...
        # Implementation using explicit loops to avoid matrix shape issues
        ...
        if beta_val > 0.0: 
            beta_complex_tensor = torch.tensor(complex(beta_val, 0), dtype=torch.complex128) 
            T_out[j, j+1] = beta_complex_tensor
            T_out[j+1, j] = beta_complex_tensor
...
def complex_dot_kernel(
    ...
    buf1_real = tl.load(buf1_ptr + offsets * 2, mask=mask, other=0.0)
    buf1_imag = tl.load(buf1_ptr + offsets * 2 + 1, mask=mask, other=0.0)
    
    v_real = tl.load(v_ptr + offsets * 2, mask=mask, other=0.0)
    v_imag = tl.load(v_ptr + offsets * 2 + 1, mask=mask, other=0.0)
...
def complex_axpy_kernel(
    ...
    buf1_real = buf1_real - scale_times_v_real
    buf1_imag = buf1_imag - scale_times_v_imag
    
    tl.store(buf1_ptr + offsets * 2, buf1_real, mask=mask)
    tl.store(buf1_ptr + offsets * 2 + 1, buf1_imag, mask=mask)
...
class LanczosStepTriton:
    ...
    def forward(self, buf1, V, T, j, m):
        ...
        # Use loop-based approach for orthogonalization
        h = torch.zeros(j+1, dtype=buf1_out.dtype, device=device)
        for i in range(j+1):
            h_real = torch.zeros(1, dtype=torch.float64, device=device)
            h_imag = torch.zeros(1, dtype=torch.float64, device=device)
            
            complex_dot_kernel[grid](
                buf1_out.data_ptr(), V[i].data_ptr(), 
                h_real.data_ptr(), h_imag.data_ptr(),
                n, buf1_out.stride(0), V.stride(1),
                BLOCK_SIZE
            )
            
            h[i] = torch.complex(h_real.item(), h_imag.item())
