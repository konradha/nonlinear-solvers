...
int main(int argc, char** argv) {
    try {
        ...
        if (args.dim == 2) {
            // Create 2D solver
            auto solver = SolverFactory::create_real_wave_solver<2>(args);
            
            // Initialize solver
            if (!solver->initialize(args.initial_u_file, args.initial_v_file)) {
                std::cerr << "Failed to initialize solver" << std::endl;
                return 1;
            }
            
            // Run simulation
            if (!solver->run(args.trajectory_file, args.velocity_file)) {
                std::cerr << "Failed to run simulation" << std::endl;
                return 1;
            }
        } else if (args.dim == 3) {
            // Create 3D solver
            auto solver = SolverFactory::create_real_wave_solver<3>(args);
