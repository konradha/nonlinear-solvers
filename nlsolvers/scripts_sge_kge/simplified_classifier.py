...
class SpectralSolitonClassifier:
    ...
    def setup_physical_grid(self):
        ...
        self.kr_physical = np.sqrt(
            self.kx_grid_physical**2 +
            self.ky_grid_physical**2)
        self.ktheta_physical = np.arctan2(
            self.ky_grid_physical, self.kx_grid_physical)
    ...
    def classify(self, u, return_details=False):
        ...
        classification, confidence_scores = self._classify_from_features(
            features)
    ...
    def _extract_key_features(self, u):
        ...
        features['spatial_spectral'] = self._analyze_spatial_spectrum(
            spatial_fft_seq)
        features['temporal_spectral'] = self._analyze_temporal_spectrum(
            temporal_fft)
    ...
    def _apply_spectral_window(self, u):
        window_x = 0.5 * \
            (1 - np.cos(2 * np.pi * np.arange(self.nx) / (self.nx - 1)))
        window_y = 0.5 * \
            (1 - np.cos(2 * np.pi * np.arange(self.ny) / (self.ny - 1)))
    ...
    def _analyze_spatial_spectrum(self, spatial_fft_seq):
        ...
        peak_analysis = self._analyze_spectral_peaks(
            radial_profile, angular_profile)
    ...
    def _compute_angular_profile(self, power_spectrum):
        ...
        for i in range(n_bins):
            ...
            mask = (
                self.ktheta_physical >= theta_min) & (
                self.ktheta_physical < theta_max)

            if np.any(mask):
                angular_profile[i] = np.mean(power_spectrum[mask])

        smoothed_profile = gaussian_filter(
            angular_profile, sigma=1.0, mode='wrap')
    ...
    def _analyze_spectral_peaks(self, radial_profile, angular_profile):
        ...
        radial_peaks, radial_props = signal.find_peaks(
            filtered_radial, height=0.05 * np.max(filtered_radial), distance=2)

        angular_peaks, angular_props = signal.find_peaks(
            angular_data, height=0.05 * np.max(angular_data), distance=3)
    ...
    def _detect_orientation_strength(self, power_spectrum):
        angular_data = self._compute_angular_profile(power_spectrum)[
            'smoothed']

        angular_data_normalized = angular_data / (np.max(angular_data) + 1e-10)

        peaks, _ = signal.find_peaks(
            angular_data_normalized, height=0.3, distance=5)
    ...
    def _analyze_temporal_spectrum(self, temporal_fft):
        ...
        peaks, props = signal.find_peaks(
            mean_temporal_power, height=0.1 * np.max(mean_temporal_power), distance=3)
        ...
        if peak_count > 0:
            ...
            dominant_freq = self.omega_physical[dominant_idx] if hasattr(
                self, 'omega_physical') else dominant_idx / self.nt
    ...
    def _compute_dynamical_properties(self, u):
        ...
        energy_concentration = self._compute_energy_concentration(
            energy_density)

        total_energy = np.sum(energy_density, axis=(1, 2)) * self.dx * self.dy
        energy_conservation = np.std(
            total_energy) / np.mean(total_energy) if np.mean(total_energy) > 0 else 0
    ...
    def _track_energy_center(self, energy_density):
        ...
        for t in range(self.nt):
            ...
            energy_center_x[t] = np.sum(
                energy_density[t] * x_grid) / total_energy
            energy_center_y[t] = np.sum(
                energy_density[t] * y_grid) / total_energy
    ...
    def _compute_energy_concentration(self, energy_density):
        ...
        r_grid = np.sqrt((self.X - np.mean(self.X))**2 +
                         (self.Y - np.mean(self.Y))**2)

        energy_radius = np.zeros(self.nt)
        for t in range(self.nt):
            energy_radius[t] = np.sum(
                r_grid * energy_density[t]) / (np.sum(energy_density[t]) + 1e-10)

        if self.nt > 1:
            radius_change_rate = (
                energy_radius[-1] - energy_radius[0]) / (self.nt * self.dt)
        else:
    ...
    def _evaluate_conservation_laws(self, u):
        ...
        total_energy = np.sum(energy_density[valid_t_slice, valid_x_slice, valid_y_slice], axis=(
            1, 2)) * self.dx * self.dy
        total_momentum_x = np.sum(
            momentum_density_x[valid_t_slice, valid_x_slice, valid_y_slice], axis=(1, 2)) * self.dx * self.dy
        total_momentum_y = np.sum(
            momentum_density_y[valid_t_slice, valid_x_slice, valid_y_slice], axis=(1, 2)) * self.dx * self.dy

        energy_deviation = np.std(
            total_energy) / np.mean(total_energy) if np.mean(total_energy) > 0 else 0
        momentum_deviation = np.std(
            np.sqrt(
                total_momentum_x**2 +
                total_momentum_y**2))
    ...
    def _compute_energy_distribution(self, u):
        ...
        total_gradient = np.sum(
            gradient_energy, axis=(
                1, 2)) * self.dx * self.dy
        total_potential = np.sum(
            potential_energy, axis=(
                1, 2)) * self.dx * self.dy

        kinetic_ratio = np.mean(
            total_kinetic) / (np.mean(total_kinetic + total_gradient + total_potential) + 1e-10)
        gradient_ratio = np.mean(
            total_gradient) / (np.mean(total_kinetic + total_gradient + total_potential) + 1e-10)
        potential_ratio = np.mean(
            total_potential) / (np.mean(total_kinetic + total_gradient + total_potential) + 1e-10)
    ...
    def _calculate_kink_score(self, spectral_entropy, velocity, straightness, orientation_strength,
        ...
        if spectral_entropy < 0.6 and velocity > 0.1 * \
                self.typical_scales['kink_velocity']:
            score = (1.0 - spectral_entropy) * (velocity /
                                                self.typical_scales['kink_velocity'])
    ...
    def _calculate_kink_antikink_score(self, spectral_entropy, radial_peak_count, velocity, straightness,
        ...
        if spectral_entropy < 0.7 and radial_peak_count >= 2 and velocity > 0.05 * \
                self.typical_scales['kink_velocity']:
            score = (1.0 - spectral_entropy) * \
                min(1.0, radial_peak_count / 2.0)
...
def compute_curl_field(u, classifier):
    ...
    dy_u[:,
         1:classifier.ny - 1] = (u[:,
                                   2:classifier.ny] - u[:,
                                                        0:classifier.ny - 2]) / (2 * classifier.dy)

    curl = np.zeros((classifier.nx, classifier.ny))

    dx_dy = (dy_u[2:classifier.nx,
                  1:classifier.ny - 1] - dy_u[0:classifier.nx - 2,
                                              1:classifier.ny - 1]) / (2 * classifier.dx)
    dy_dx = (dx_u[1:classifier.nx - 1,
                  2:classifier.ny] - dx_u[1:classifier.nx - 1,
                                          0:classifier.ny - 2]) / (2 * classifier.dy)
...
class SolitonVisualizer:
    def __init__(self, classifier, save_dir="soliton_analysis"):
        ...
        self.figure_dpi = 300
        self.class_colors = sns.color_palette(
            "Set1", len(classifier.class_names))
    ...
    def visualize_and_save(self, u, name=None):
        ...
        classification, confidence_scores, features = self.classifier.classify(
            u, return_details=True)
        ...
        self._plot_classification_results(
            classification, confidence_scores, name)
    ...
    def _plot_spatial_field(self, u, name, classification):
        ...
        im = ax.imshow(u[mid_t], cmap=self.cmap_diverging, origin='lower',
                       extent=[-self.classifier.Lx, self.classifier.Lx, -self.classifier.Ly, self.classifier.Ly])
        ax.set_title(
            f"Mid-simulation Field (t={mid_t*self.classifier.dt:.2f})")
        ax.set_xlabel("x")
        ax.set_ylabel("y")
        divider = make_axes_locatable(ax)
        cax = divider.append_axes("right", size="5%", pad=0.05)
        ...
        plt.savefig(
            f"{self.save_dir}/{name}_spatial_field.png",
            dpi=self.figure_dpi)
        plt.close()
    ...
    def _plot_energy_distribution(self, features, name):
        ...
        ax = axes[0, 0]
        energy_ratios = [
            energy_data["kinetic_ratio"],
            energy_data["gradient_ratio"],
            energy_data["potential_ratio"]]
        ...
        ax.plot(
            time_steps,
            energy_data["kinetic_profile"],
            'r-',
            label='Kinetic')
        ax.plot(
            time_steps,
            energy_data["gradient_profile"],
            'g-',
            label='Gradient')
        ax.plot(
            time_steps,
            energy_data["potential_profile"],
            'b-',
            label='Potential')
        ...
        ax = axes[1, 1]
        total_energy = energy_data["kinetic_profile"] + \
            energy_data["gradient_profile"] + energy_data["potential_profile"]
        ax.plot(time_steps, total_energy, 'k-', linewidth=2)
        ax.set_xlabel('Time Step')
        ax.set_ylabel('Total Energy')
        ax.set_title(
            f'Total Energy (Deviation: {features["conservation_laws"]["energy_deviation"]:.4f})')

        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(
            f"{self.save_dir}/{name}_energy_distribution.png",
            dpi=self.figure_dpi)
        plt.close()
    ...
    def _plot_spectral_analysis(self, features, name):
        ...
        im = ax.imshow(
            ...
            origin='lower')
        ax.set_title(
            f"Power Spectrum (Entropy: {spatial_spectral['spectral_entropy']:.4f})")
        ...
        ax.set_title(
            f'Energy Center Trajectory (v={energy_center["velocity"]:.4f})')
        ax.grid(True)
        ax.legend()

        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(
            f"{self.save_dir}/{name}_spectral_analysis.png",
            dpi=self.figure_dpi)
        plt.close()
    ...
    def _plot_temporal_dynamics(self, features, name):
        ...
        ax.set_ylabel('Power')
        ax.set_title(
            f'Temporal Power Spectrum (Peaks: {temporal["peak_count"]})')
        ...
        ax.set_title(
            f'Energy Radius (Change Rate: {dynamical["energy_concentration"]["radius_change_rate"]:.4f})')

        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(
            f"{self.save_dir}/{name}_temporal_dynamics.png",
            dpi=self.figure_dpi)
        plt.close()

    def _plot_classification_results(
            self, classification, confidence_scores, name):
        ...
        bars = ax.barh(y_pos, confidence_scores, align='center', color=[
                       self.class_colors[i] for i in range(len(self.classifier.class_names))])
        ax.set_yticks(y_pos)
        ...
        ax = axes[1]
        characteristic_features = self._get_characteristic_features(
            classification, confidence_scores, self.classifier.class_names)
        ...
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(
            f"{self.save_dir}/{name}_classification.png",
            dpi=self.figure_dpi)
        plt.close()

    def _get_characteristic_features(
            self, classification, confidence_scores, class_names):
    ...
    def create_dashboard(self, u, name=None):
        ...
        ax.set_ylabel('Y Position')
        ax.set_title(
            f'Energy Center Trajectory (v={features["dynamical"]["energy_center"]["velocity"]:.4f})')
        ax.grid(True)
        ax.legend()
    ...
    def _plot_key_differentiating_features(self, ax, features, classification):
        ...
        if classification == "Kink":
            ...
            feature_values = [
                ...
                self.classifier.typical_scales["kink_velocity"],
                features["dynamical"]["energy_center"]["straightness"],
                features["spatial_spectral"]["orientation_strength"],
                min(1.0, features["spatial_spectral"]
                    ["peak_analysis"]["radial_peak_count"] / 2.0),
            ...
            feature_values = [
                min(1.0, features["spatial_spectral"]
                    ["peak_analysis"]["radial_peak_count"] / 3.0),
                min(1.0, features["spatial_spectral"]
                    ["peak_analysis"]["angular_peak_count"] / 3.0),
                features["spatial_spectral"]["orientation_strength"],
            ...
            feature_values = [
                ...
                min(1.0, features["dynamical"]
                    ["energy_concentration"]["ratio"] / 5.0),
                features["energy_distribution"]["kinetic_ratio"] * 2,
                1.0 - features["temporal_spectral"]["entropy"],
                abs(features["dynamical"]["energy_concentration"]
                    ["radius_change_rate"]) * 10
            ]

        y_pos = np.arange(len(key_features))
        ax.barh(y_pos, feature_values, align='center',
                color=self.class_colors[self.classifier.class_names.index(classification)])
        ax.set_yticks(y_pos)
...
def create_comparative_analysis(
        solutions, classifier, save_dir="soliton_analysis"):
    ...
    ax.set_xticklabels([name[:10]
                       for name in solution_names], rotation=45, ha="right")
