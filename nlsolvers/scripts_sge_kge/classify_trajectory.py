...
class SolitonDashboard:
    def __init__(self, nx, ny, Lx, Ly, dt, T, save_dir):
    ...
    def create_dashboard(self, u, v=None, name=None):
        ...
        plt.savefig(f"{self.save_dir}/{name or 'dashboard'}.png", bbox_inches='tight', dpi=300)
        plt.close()
    ...
    def _plot_analysis(self, fig, gs, features):
        ...
        ax2 = fig.add_subplot(subgs[0,1])
        ax2.imshow(np.log10(features['power_spectrum']+1e-10), cmap='viridis')
        ax2.set_title("Power Spectrum")

        ax3 = fig.add_subplot(subgs[0,2])
        ax3.bar([1,2,3], [features['localization'], features['symmetry'], features['stability']])
    ...
    def _compute_terms(self, u):
        ...
        nonlinear = [np.sum(np.sin(ut)**2)*self.dx*self.dy for ut in u]
...
def batch_process_solutions(solution_dict, nx, ny, Lx, Ly, dt, T, save_dir=None):
    dashboard = SolitonDashboard(nx, ny, Lx, Ly, dt, T, save_dir)
    results = {}
