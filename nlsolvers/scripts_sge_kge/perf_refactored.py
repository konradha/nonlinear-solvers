...
def compute_structure_similarity(data, reference_frame=None):
    ...
    for t in range(nt):
        ssim_values[t] = ssim(reference_frame, data[t], data_range=data.max()-data.min())
...
def compute_local_conservation(data, vel_data, dx, dy, dt):
    ...
    for t in range(1, nt):
        ...
        laplacian = (u[2:, 1:-1] + u[:-2, 1:-1] + u[1:-1, 2:] + u[1:-1, :-2] - 4*u[1:-1, 1:-1]) / (dx*dy)
        ut_numerical = (u - u_prev) / dt
        ut_physical = v
        
        conservation_err = np.mean(np.abs(ut_numerical[1:-1, 1:-1] - ut_physical[1:-1, 1:-1]))
...
class IntegratorComparison:
    ...
    def generate_base_conditions(self):
        ...
        phenomenon_params = self.sample_phenomenon_params(self.args.phenomenon_type)
        ...
        m = 5. * create_periodic_boxes(max_nx, self.Lx) #np.ones((max_nx, max_nx))
    ...
    def run_simulation(self, integrator_type, nx, nt, u0, v0, m):
        ...
        if nx != u0.shape[0]:
            u0_nx = downsample_interpolation(u0.reshape(1, *u0.shape), (nx, nx), self.Lx, self.Ly)[0]
            v0_nx = downsample_interpolation(v0.reshape(1, *v0.shape), (nx, nx), self.Lx, self.Ly)[0]
            m_nx = downsample_interpolation(m.reshape(1, *m.shape), (nx, nx), self.Lx, self.Ly)[0]
        else:
        ...
        vel_file = self.traj_dir / f"{integrator_type}_{nx}_{nt}_{self.run_id}_vel.npy"
    ...
    def analyze_simulation(self, integrator, nx, nt, traj_file, vel_file):
        ...
        dy = 2 * self.Ly / (nx - 1)
        dt = self.T / (nt - 1)
        energy_components = [compute_energy(u, vel_data[i], dx, dy) for i, u in enumerate(traj_data)]
        ...
        k_centers, dispersion_radial, dispersion_std, dispersion_full = compute_spectral_dispersion(traj_data, dx, dt)
        
        conservation_metric = np.array(
            [np.nan] + [np.log(np.abs(total_energy[i] - total_energy[0])) for i in range(1, snapshots)]
            )
    ...
    def plot_energy_comparison(self, nt, metrics_by_nx):
        ...
        for i, nx in enumerate(sorted(metrics_by_nx.keys())):
            ...
            for integrator, metrics in metrics_by_nx[nx].items():
                plt.plot(metrics['time_points'], metrics['total_energy'], label=f"{integrator}")
            ...
            for integrator, metrics in metrics_by_nx[nx].items():
                plt.plot(metrics['time_points'], metrics['energy_drift'], label=f"{integrator}")
        ...
        if len(list(metrics_by_nx.keys())):
            ...
            plt.savefig(self.plots_dir / f"energy_comparison_nt{nt}_nx{nx}_{self.run_id}.png", dpi=300)
        else:
            plt.savefig(self.plots_dir / f"energy_comparison_nt{nt}_{self.run_id}.png", dpi=300)
        plt.close()
    ...
    def plot_structure_comparison(self, nt, metrics_by_nx):
        ...
        for i, nx in enumerate(sorted(metrics_by_nx.keys())):
            plt.subplot(len(metrics_by_nx), 2, 2*i+1)
            for integrator, metrics in metrics_by_nx[nx].items():
                plt.plot(metrics['time_points'], metrics['structure_similarity'], label=f"{integrator}")
            ...
            plt.subplot(len(metrics_by_nx), 2, 2*i+2)
            for integrator, metrics in metrics_by_nx[nx].items():
                plt.plot(metrics['time_points'], metrics['conservation_metric'], label=f"{integrator}")
        ...
        if len(list(metrics_by_nx.keys())):
            nx = list(metrics_by_nx.keys())[0]
            plt.savefig(self.plots_dir / f"structure_comparison_nt{nt}_nx{nx}_{self.run_id}.png", dpi=300)
        else:
            plt.savefig(self.plots_dir / f"structure_comparison_nt{nt}_{self.run_id}.png", dpi=300)
        plt.close()
    
    def plot_modal_energy(self, nx, nt, metrics_by_integrator):
        plt.figure(figsize=(12, 10)) 
        for i, (integrator, metrics) in enumerate(metrics_by_integrator.items()):
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / f"modal_energy_nx{nx}_nt{nt}_{self.run_id}.png", dpi=300)
        plt.close()
    ...
    def plot_dispersion_relation(self, nx, nt, metrics_by_integrator):
        ...
        for integrator, metrics in metrics_by_integrator.items():
            ...
            plt.fill_between(k_values, dispersion-dispersion_std, dispersion+dispersion_std, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / f"dispersion_nx{nx}_nt{nt}_{self.run_id}.png", dpi=300)
        plt.close()
    
    def plot_metric_vs_resolution(self, metrics_all):
        metrics_to_plot = [
            ('energy_drift', 'Max Energy Drift', lambda x: np.max(x['energy_drift'])),
            ('structure_similarity', 'Final Structure Similarity', lambda x: x['structure_similarity'][-1]),
            ('conservation_metric', 'Mean Conservation Error', lambda x: np.mean(x['conservation_metric']))
        ]
        
        for nt in self.nt_values:
            plt.figure(figsize=(15, 5))
            
            for i, (metric_key, metric_name, metric_func) in enumerate(metrics_to_plot):
                plt.subplot(1, 3, i+1)
                ...
                for integrator in ['gautschi', 'sv']:
                    ...
                    for nx in self.nx_values:
                        if (nx, nt) in metrics_all and integrator in metrics_all[(nx, nt)]:
                            x_values.append(nx)
                            y_values.append(metric_func(metrics_all[(nx, nt)][integrator]))
            ...
            plt.tight_layout()
            plt.savefig(self.plots_dir / f"metrics_vs_nx_nt{nt}_{self.run_id}.png", dpi=300)
            plt.close()
        ...
        for nx in self.nx_values:
            ...
            plt.tight_layout()
            plt.savefig(self.plots_dir / f"metrics_vs_nt_nx{nx}_{self.run_id}.png", dpi=300)
            plt.close()
    
    def plot_detailed_comparison(self, nx, nt, gautschi_traj, sv_traj, gautschi_metrics, sv_metrics):
        time_points = gautschi_metrics['time_points']
        
        plt.figure(figsize=(15, 10))
        
        plt.subplot(2, 3, 1)
        plt.plot(time_points, gautschi_metrics['energy_gradient'], label='Gradient (G)')
        plt.plot(time_points, gautschi_metrics['energy_kinetic'], label='Kinetic (G)')
        plt.plot(time_points, gautschi_metrics['energy_potential'], label='Potential (G)')
        ...
        plt.subplot(2, 3, 2) 
        plt.plot(time_points, sv_metrics['energy_gradient'], '--', label='Gradient (SV)')
        plt.plot(time_points, sv_metrics['energy_kinetic'], '--', label='Kinetic (SV)')
        plt.plot(time_points, sv_metrics['energy_potential'], '--', label='Potential (SV)')
        ...
        plt.subplot(2, 3, 3)
        plt.plot(time_points, gautschi_metrics['structure_similarity'], label='SSIM (G)')
        plt.plot(time_points, sv_metrics['structure_similarity'], '--', label='SSIM (SV)')
        ...
        for i, idx in enumerate(indices):
            plt.subplot(2, 3, i+4)
            plt.imshow(np.abs(gautschi_traj[idx] - sv_traj[idx]), cmap='viridis')
            plt.colorbar()
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / f"detailed_comparison_nx{nx}_nt{nt}_{self.run_id}.png", dpi=300)
        plt.close()
    ...
    def execute_comparison(self):
        ...
        for nx in self.nx_values:
            ...
            for nt in self.nt_values:
                ...
                for integrator in ['gautschi', 'sv']:
                    traj_file, vel_file, walltime = self.run_simulation(integrator, nx, nt, u0, v0, m)
                    print(f"    {integrator} completed in {walltime:.2f}s")
                    walltime_data[nx][nt][integrator] = walltime
                    
                    metrics, traj_data, vel_data = self.analyze_simulation(integrator, nx, nt, traj_file, vel_file)
    ...
    def plot_computational_efficiency(self, walltime_data, metrics_all):
        ...
        for nx in self.nx_values:
            ...
            gautschi_times = [walltime_data[nx][nt]['gautschi'] for nt in self.nt_values]
            plt.plot(self.nt_values, sv_times, 'o-', label=f'SV nx={nx}')
            plt.plot(self.nt_values, gautschi_times, 's--', label=f'Gautschi nx={nx}')
        ...
        for nt in self.nt_values:
            ...
            gautschi_times = [walltime_data[nx][nt]['gautschi'] for nx in self.nx_values]
            plt.plot(self.nx_values, sv_times, 'o-', label=f'SV nt={nt}')
            plt.plot(self.nx_values, gautschi_times, 's--', label=f'Gautschi nt={nt}')
        ...
        for nt in self.nt_values:
            ...
            for nx in self.nx_values:
                if (nx, nt) in metrics_all and 'sv' in metrics_all[(nx, nt)] and 'gautschi' in metrics_all[(nx, nt)]:
                    sv_accuracy = 1.0 / (1e-10 + np.max(metrics_all[(nx, nt)]['sv']['energy_drift']))
                    gautschi_accuracy = 1.0 / (1e-10 + np.max(metrics_all[(nx, nt)]['gautschi']['energy_drift']))
                    
                    sv_efficiency.append(sv_accuracy / walltime_data[nx][nt]['sv'])
                    gautschi_efficiency.append(gautschi_accuracy / walltime_data[nx][nt]['gautschi'])
                    nx_values.append(nx)
            
            if nx_values:
                plt.plot(nx_values, sv_efficiency, 'o-', label=f'SV nt={nt}')
                plt.plot(nx_values, gautschi_efficiency, 's--', label=f'Gautschi nt={nt}')
        ...
        for nt in self.nt_values:
            ...
            for nx in self.nx_values:
                if (nx, nt) in metrics_all and 'sv' in metrics_all[(nx, nt)] and 'gautschi' in metrics_all[(nx, nt)]:
                    sv_accuracy = metrics_all[(nx, nt)]['sv']['structure_similarity'][-1]
                    gautschi_accuracy = metrics_all[(nx, nt)]['gautschi']['structure_similarity'][-1]
        ...
        plt.tight_layout()
        plt.savefig(self.plots_dir / f"computational_efficiency_{self.run_id}.png", dpi=300)
        plt.close()

def parse_args():
    parser = argparse.ArgumentParser(description="Compare Gautschi and Störmer-Verlet integrators")
    ...
    parser.add_argument("--system-type", type=str, required=True,
                        choices=["klein_gordon", "sine_gordon", "double_sine_gordon", "hyperbolic_sine_gordon"],
