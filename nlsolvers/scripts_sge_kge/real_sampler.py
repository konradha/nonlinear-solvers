...
class RealWaveSampler:
    ...
    def nonlinear_potential(self, u: np.ndarray, system_type: str = 'sine_gordon') -> np.ndarray:
    ...
    def kink_solution(self, system_type: str = 'sine_gordon', width: float = 1.0,
        ...
        if system_type == 'sine_gordon':
            ...
            if kink_type == 'anti':
                ...
                u = np.tanh(X_rot / width) - np.tanh((X_rot - 4 * width) / width)
                if velocity_type == 'fitting':
                    v = vx / (width * (np.cosh(X_rot / width)**2)) - vx / (width * (np.cosh((X_rot - 4 * width) / width)**2))
                else:
                    v = np.zeros_like(u)
            else: # fallback + 'standard'
                u = np.tanh(X_rot / width)
    ...
    def kink_field(self, system_type: str = 'sine_gordon', winding_x: int = 1, winding_y: int = 0,
        ...
        v0 = self.anisotropic_grf(length_scale=np.mean(width_range) * 2.0, amplitude=np.max(np.abs(u0)) * 0.1)
    ...
    def multi_breather_field(self, system_type: str = 'sine_gordon', num_breathers: int = 3,
                            ...
                            frequency_range: Tuple[float, float] = (0.6, 0.95),
        ...
        if position_type not in ['circle', 'line']:
            ...
            for i in range(num_breathers):
                pos = -self.L + 2 * self.L * i / (num_breathers - 1 if num_breathers > 1 else 1)
        ...
        for x0, y0 in positions:
            width = width_range[0] + (width_range[1] - width_range[0]) * np.random.rand()
    ...
    def multi_spiral_state(self, n_spirals: int = 3, amplitude_range: Tuple[float, float] = (0.5, 1.5),
        ...
        for i in range(n_spirals):
            amplitude = amplitude_range[0] + (amplitude_range[1] - amplitude_range[0]) * np.random.rand()
            num_arms = np.random.randint(num_arms_range[0], num_arms_range[1] + 1)
            decay_rate = decay_rate_range[0] + (decay_rate_range[1] - decay_rate_range[0]) * np.random.rand()
    ...
    def ring_soliton(self, system_type: str = 'sine_gordon', amplitude: float = 1.0,
        ...
        if ring_type == 'expanding':
            if system_type == 'sine_gordon':
                ...
                u = 4 * np.arctan(np.exp((radius - r_local) / width)) - 2 * np.pi
                v = -velocity * 4 / (width * (np.cosh((radius - r_local) / width)**2))
            elif system_type == 'klein_gordon':
                u = amplitude * np.tanh((radius - r_local) / width)
                v = -velocity * amplitude / (width * (np.cosh((radius - r_local) / width)**2))
            ...
            if system_type == 'sine_gordon':
                ...
                u = 4 * np.arctan(np.exp((inner_radius - r_local) / (width/2))) - 4 * np.arctan(np.exp((outer_radius - r_local) / (width/2))) - 2 * np.pi
                v = -velocity * 4 / (width/2 * (np.cosh((inner_radius - r_local) / (width/2))**2)) + velocity * 4 / (width/2 * (np.cosh((outer_radius - r_local) / (width/2))**2))
            elif system_type == 'klein_gordon':
                u = amplitude * np.tanh((inner_radius - r_local) / (width/2)) - amplitude * np.tanh((outer_radius - r_local) / (width/2))
                v = -velocity * amplitude / (width/2 * (np.cosh((inner_radius - r_local) / (width/2))**2)) + velocity * amplitude / (width/2 * (np.cosh((outer_radius - r_local) / (width/2))**2))
    ...
    def multi_ring_state(self, system_type: str = 'sine_gordon', n_rings: int = 3,
        ...
        for i, (x0, y0) in enumerate(positions):
            ...
            if modulation_strength > 0:
                mod_mode = np.random.randint(modulation_mode_range[0], modulation_mode_range[1] + 1)
            else:
                mod_mode = 0
    ...
    def skyrmion_lattice(self, system_type: str = 'sine_gordon', n_skyrmions: int = 5,
        ...
        if charge_distribution == 'alternating':
            ...
            charges = [1 if np.random.rand() > 0.5 else -1 for _ in range(n_skyrmions)]

        for i, ((x0, y0), charge) in enumerate(zip(positions, charges)):
            radius = radius_range[0] + (radius_range[1] - radius_range[0]) * np.random.rand()
            profile = ['standard', 'compact', 'exponential'][np.random.randint(0, 3)]
    ...
    def skyrmion_like_field(self, num_skyrmions: int = 1) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def breather_solution(self, system_type: str = 'sine_gordon', amplitude: float = 0.5,
        ...
        if system_type == 'sine_gordon':
            ...
            u = amplitude * np.exp(-(xi**2) / 2) * np.cos(frequency * tau + phase)
            if velocity_type == 'fitting':
                v = -amplitude * frequency * np.exp(-(xi**2) / 2) * np.sin(frequency * tau + phase)
            else:
                v = np.zeros_like(u)
    ...
    def soliton_antisoliton_pair(self, system_type: str = 'sine_gordon',
        ...
        if pattern_type == 'auto':
            pattern_type = np.random.choice(['radial', 'linear', 'angular', 'nested'])
        ...
        if pattern_type == 'radial':
            r = np.sqrt((self.X - x0)**2 + (self.Y - y0)**2)
            u0 = 4*np.arctan(np.exp(r/width)) - 4*np.arctan(np.exp((r - 0.5*width)/width))
        elif pattern_type == 'linear':
            theta = np.pi*np.random.rand()
            x_rot = (self.X - x0)*np.cos(theta) + (self.Y - y0)*np.sin(theta)
            u0 = 4*np.arctan(np.exp(x_rot/width)) - 4*np.arctan(np.exp(-x_rot/width))
        elif pattern_type == 'angular':
            phi = np.arctan2(self.Y - y0, self.X - x0)
            u0 = 4*np.arctan(np.exp(np.sin(phi)/width)) - 4*np.arctan(np.exp(-np.sin(phi)/width))
        else:
            r1 = 0.3*self.L + 0.1*self.L*np.random.rand()
            r2 = 0.6*self.L + 0.1*self.L*np.random.rand()
            r = np.sqrt((self.X - x0)**2 + (self.Y - y0)**2)
            u0 = 4*np.arctan(np.exp((r-r1)/width)) - 4*np.arctan(np.exp((r-r2)/width))

        v0 = self.anisotropic_grf(length_scale=width, anisotropy_ratio=2.0) * 0.2
    ...
    def elliptical_soliton(self, system_type: str = 'sine_gordon',
        ...
        if complexity != 'complex':
            ...
            X_rot = (self.X - x0)*np.cos(theta) + (self.Y - y0)*np.sin(theta)
            Y_rot = -(self.X - x0)*np.sin(theta) + (self.Y - y0)*np.cos(theta)
    ...
    def generate_sample(self, system_type: str = 'sine_gordon', phenomenon_type: str = 'kink_solution',
        ...
        if phenomenon_type == 'kink_solution':
            ...
            return self.multi_breather_field(system_type, time_param=time_param, velocity_type=velocity_type, **params)
        elif phenomenon_type == 'ring_soliton':
            return self.ring_soliton(system_type, time_param=time_param, **params)
            ...
            return self.q_ball_solution(system_type, time_param=time_param, **params)
        elif phenomenon_type == 'multi_q_ball':
            return self.multi_q_ball(system_type, time_param=time_param, **params)
            ...
            for param, range_values in parameter_ranges.items():
                if isinstance(range_values, list):
                    ...
                    if isinstance(range_values[0], int) and isinstance(range_values[1], int):
                        ...
                        params[param] = range_values[0] + (range_values[1] - range_values[0]) * np.random.rand()
                else:
    ...
    def generate_diverse_ensemble(self, system_type: str = 'sine_gordon', phenomenon_type: str = 'kink_solution',
        ...
        def diversity_distance(s1, s2):
            ...
            if diversity_metric == 'l2':
                ...
                u_overlap = np.sum(s1_u_fft * s2_u_fft) / (s1_u_fft_norm * s2_u_fft_norm)
                v_overlap = np.sum(s1_v_fft * s2_v_fft) / (s1_v_fft_norm * s2_v_fft_norm)
    ...
    def generate_initial_condition(self, system_type: str = 'sine_gordon',
        ...
        if phenomenon_type is None: raise Exception 
