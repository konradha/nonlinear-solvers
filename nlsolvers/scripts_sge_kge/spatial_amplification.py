...
def create_periodic_boxes(n, L, factor=1.5, num_boxes_per_dim=3,
    ...
    spacing = (available_space - total_boxes_length) / \
        (num_boxes_per_dim - 1) if num_boxes_per_dim > 1 else 0
...
def create_periodic_gaussians(n, L, factor=1.5, num_gaussians_per_dim=3,
    ...
    spacing = available_space / \
        (num_gaussians_per_dim - 1) if num_gaussians_per_dim > 1 else 0
    ...
    for i in range(num_gaussians_per_dim):
        for j in range(num_gaussians_per_dim):
            ...
            gaussian = np.exp(-((X - x_center)**2 +
                              (Y - y_center)**2) / (2 * sigma**2))
            if change_sign:
                m += -factor * gaussian
            else:
                m += factor * gaussian
...
def create_grf(nx, ny, Lx, Ly, mean=1.0, std=0.5, scale=2.0):
    ...
    smoothed = gaussian_filter(
        white_noise, sigma=scale * min(nx, ny) / (2 * max(Lx, Ly)))
...
def create_wavelet_modulated_grf(nx, ny, Lx, Ly, wavelet_scale=1.0,
    ...
    wavelet_sum = (wavelet_sum - np.min(wavelet_sum)) / \
        (np.max(wavelet_sum) - np.min(wavelet_sum))
...
def scale_m_to_range(m, min_val=1.0, max_val=2.0):
    ...
    m_scaled = (m - current_min) / (current_max - current_min) * \
