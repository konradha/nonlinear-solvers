...
class Launcher:
    def __init__(self, args):
        ...
        self.sampler = NLSESampler(args.nx, args.ny, args.nz, args.Lx)
    ...
    def generate_initial_condition(self, run_idx, phenomenon_params=None):
        ...
        u0 = self.sampler.generate_initial_condition(
    ...
    def generate_animation_title(self, phenomenon_params, walltime):
        ...
        if "cubic":
            str_start = "NLSE: $i u_t + div(c(x,y,z) \\nabla u ) + m(x,y,z) |u|^2 u = 0$\n"
        else:
    ...
    def cleanup(self, run_idx):
        if self.args.delete_intermediates:
            ...
            for file in [u0_file, m_file, traj_file]:
                if file.exists():
                    os.unlink(file)
    ...
    def run(self):
        ...
        for i in range(self.args.num_runs):
            try:
                ...
                sols[f"{self.run_id}_{i}"] = traj_data
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="NLSE solver launcher (3D): i u_t + div(c(x,y,z) grad(u)) + m(x,y,z) |u|² u = 0")

    parser.add_argument("--system-type", type=str, default="cubic",
                        choices=["cubic"])

    parser.add_argument("--phenomenon", type=str, default="multi_soliton_state",
                        choices=["skyrmion_tube", "multi_soliton_state"],
