...
class NLSELauncher:
    ...
    def save_parameter_file(self):
        ...
        with open(params_file, "w") as f:
            ...
            f.write(f"Time: T={self.args.T}, steps={self.args.nt}, snapshots={self.args.snapshots}\n")
    ...
    def generate_initial_condition(self, run_idx, phenomenon_params=None):
        ...
        # scale to max amplitude 1
        u0 = u0 / np.max(np.abs(u0))
        
        #u0 = u0
        #current_norm = np.sum(np.abs(u0)**2) * self.dV
        #if current_norm > 0:
    ...
    def sample_phenomenon_params(self): 
        ...
        if self.args.phenomenon in parameter_spaces:
            ...
            raise ValueError(f"Unknown phenomenon type: {self.args.phenomenon}")
    ...
    def generate_spatial_amplification(self, run_idx):
        ...
        m = scale_m_to_range(m, min_val=1.0, max_val=2.0)
    ...
    def downsample_trajectory(self, traj_data):
        if self.args.dr_strategy == 'none':
            ...
            return downsample_fft(traj_data, target_shape=(self.args.dr_x, self.args.dr_y))
            ...
            raise ValueError(f"Unknown downsampling strategy: {self.args.dr_strategy}")
    
    def save_to_hdf5(self, run_idx, u0, m, traj_data, phenomenon_params, elapsed_time):
    ...
    def create_visualizations(self, run_idx, traj_data, m, phenomenon_params, walltime):
        # TODO
        #fig = create_analysis_plots(traj_data, m, self.args)
        #fig.savefig(self.analysis_dir / f"analysis_{self.run_id}_{run_idx:04d}.png", dpi=300)
        #plt.close(fig)
    ...
    def generate_animation_title(self, phenomenon_params, walltime):
        ...
        params_str = ", ".join([f"{k}={v}" for k, v in phenomenon_params.items() if not isinstance(v, (list, tuple))])
        if len(params_str) > 100:
            params_str = params_str[:97] + "..."
        
        return (
            f"NLSE iu_t + Δu + m(x,y)|u|²u = 0\n"
            f"{self.args.phenomenon}, m: {self.args.m_type}; {m_string}\n"
            f"domain: [0, T={self.args.T}] x [-{self.args.Lx:.2f}, {self.args.Lx:.2f}] x [-{self.args.Ly:.2f}, {self.args.Ly:.2f}]\n"
            #f"phenomenon: {self.args.phenomenon}, params: {params_str}\n"
            #f"boundary conditions: no-flux\n"
    ...
    def run(self):
        for i in range(self.args.num_runs):
            try:
                ...
                # TODO
                # fig = create_analysis_plots(traj_data, m, self.args)
                # fig.savefig(self.analysis_dir / f"analysis_{self.run_id}_{i:04d}.png", dpi=300)
                # plt.close(fig)
                
                traj_data = self.downsample_trajectory(traj_data)
                self.save_to_hdf5(i, u0, m, traj_data, phenomenon_params, walltime)
                self.create_visualizations(i, traj_data, m, phenomenon_params, walltime)
                self.cleanup(i)
                
                print(f"Completed run {i+1}/{self.args.num_runs} ({self.args.phenomenon}, {self.args.m_type})")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Advanced NLSE cubic nonlinearity solver launcher")
    ...
    parser.add_argument("--seed", type=int, default=0xdeadbeef, help="Random seed")
