...
__global__ void transform_cos_sqrt(double* transform,
...
__global__ void transform_id_sqrt(double* transform,
                                const double* eigenvalues,
                                double t,
                                uint32_t m) {
   uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void compute_matrix_function_kernel(double* result,
                                             const double* Q,
...
__global__ void apply_to_first_column_kernel(double* result,
                                           const double* V,
                                           const double* matrix,
                                           double beta,
                                           uint32_t n,
...
public:
   ...
   MatrixFunctionApplicatorReal(const int* d_row_ptr, const int* d_col_ind, 
                               ...
                               uint32_t nnz, const Parameters& params = Parameters())
   ...
   void apply(double* result, const double* input, double t, FunctionType type) {
       ...
       cudaMemcpy(V.data(), krylov_.V, n_ * m_ * sizeof(double), cudaMemcpyDeviceToHost);
       cudaMemcpy(T.data(), krylov_.T, m_ * m_ * sizeof(double), cudaMemcpyDeviceToHost);
       cudaMemcpy(eigenvalues.data(), d_eigenvalues_, m_ * sizeof(double), cudaMemcpyDeviceToHost);
       cudaMemcpy(eigenvectors.data(), d_eigenvectors_, m_ * m_ * sizeof(double), cudaMemcpyDeviceToHost);
...
private:
   ...
   void apply_to_first_column(double* result) {
       apply_to_first_column_kernel<<<grid_dim_1d_, block_dim_1d_>>>(
           result, krylov_.V, d_work_, 
           *krylov_.d_beta, n_, m_);
   }
