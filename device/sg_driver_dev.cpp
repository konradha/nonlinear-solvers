...
int main() {
   ...
   Eigen::VectorX<f_ty> u0 = apply_function_uniform<f_ty>(-Lx, Lx, nx, -Ly, Ly, ny, f);
   Eigen::VectorX<f_ty> v0 = apply_function_uniform<f_ty>(-Lx, Lx, nx, -Ly, Ly, ny, zero);
   Eigen::VectorX<f_ty> m = apply_function_uniform<f_ty>(-Lx, Lx, nx, -Ly, Ly, ny, neg);
   Eigen::VectorX<f_ty> c = apply_function_uniform<f_ty>(-Lx, Lx, nx, -Ly, Ly, ny, one);

   const Eigen::SparseMatrix<f_ty> L = build_laplacian_noflux<f_ty>(nx - 2, ny - 2, dx, dy);
   ...
   cudaMemcpy(d_row_ptr, L.outerIndexPtr(), (L.rows() + 1) * sizeof(int), cudaMemcpyHostToDevice);
   ...
   //std::cout << "host v0[100 * nx + ny / 2]: " << v0.data()[100* nx + ny / 2] << "\n";
   auto gpu_start = std::chrono::high_resolution_clock::now();
   {
       Eigen::VectorX<double> u_past = (u0 - dt * v0);
       SGESolverDevice::Parameters params(num_snapshots, freq);
       SGESolverDevice solver(d_row_ptr, d_col_ind, d_values, 
                             c.data(), m.data(), nx * ny, L.nonZeros(),
			     u0.data(), v0.data(), u_past.data(),
                             params);
       for (uint32_t i = 1; i < nt; ++i) {
           solver.step(dt, i);
       }
