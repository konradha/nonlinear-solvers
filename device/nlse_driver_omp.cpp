// nvcc -O2 --fmad=true --restrict -std=c++17 --expt-relaxed-constexpr
// -diag-suppress 20012 -diag-suppress 20011 -lcusparse -lcublas -lcusolver
// -Xcompiler -fopenmp -x cu --extended-lambda -arch=sm_70 nlse_driver_omp.cpp
// -o to_nlse_driver_par
...
#pragma omp parallel
  {
    ...
    auto f_collision = [&](c_ty x, c_ty y) {
      auto un1 = A *
                 std::exp(-((x - x1) * (x - x1) + (y - y1) * (y - y1)) / 4. /
                          sigma / sigma) *
                 std::exp(c_ty(0, 1.) * (kx * x + ky * y));
      auto un2 = A *
                 std::exp(-((x - x2) * (x - x2) + (y - y2) * (y - y2)) / 4. /
                          sigma / sigma) *
                 std::exp(c_ty(0, -1.) * (kx * x + ky * y));
      return un1 + un2;
    };
...
#pragma omp parallel
  {
    const int thread_id = omp_get_thread_num();
    const auto fname =
        "evolution_nlse_device_" + std::to_string(thread_id) + ".npy";
  ...
  std::cout << "Average I/O overhead: "
            << (avg_io_time / avg_compute_time) * 100 << "%\n";
