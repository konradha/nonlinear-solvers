...
public:
  ...
  void apply(ScalarType *result, const double *u, TimeType t, function_type f,
             TransformType transform = TransformType::DIRECT) {
...
template <typename ScalarType, typename TimeType>
__global__ void
transform_eigenvalues_kernel(ScalarType *transformed, const double *eigenvalues,
                             TimeType t, ScalarType (*f)(TimeType, double),
                             uint32_t m) {
...
__global__ void transform_eigenvalues_sqrt_kernel(
    ScalarType *transformed, const double *eigenvalues, TimeType t,
    ScalarType (*f)(TimeType, double), uint32_t m) {
...
__global__ void
compute_matrix_function_kernel(ScalarType *result_matrix, const double *Q,
...
template <typename ScalarType>
__global__ void
apply_to_first_column_kernel(ScalarType *result, const double *V,
                             const ScalarType *matrix, const double beta,
...
template <typename ScalarType, typename TimeType>
ScalarType exp_func(TimeType t, double x) {
  return exp(t * x);
}

template <typename ScalarType, typename TimeType>
ScalarType cos_func(TimeType t, double x) {
  return cos(t * x);
}

template <typename ScalarType, typename TimeType>
ScalarType sin_func(TimeType t, double x) {
  return sin(t * x);
}
