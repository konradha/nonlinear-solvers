...
void test_matfunc(const Eigen::SparseMatrix<double> &A, uint32_t m,
    ...
    cudaMemcpy(d_values, values, A.nonZeros() * sizeof(double), cudaMemcpyHostToDevice);

    MatrixFunctionApplicatorReal::Parameters params;
    MatrixFunctionApplicatorReal matfunc(d_row_ptr, d_col_ind, d_values, n, m, A.nonZeros(), params);
    ...
    for (uint32_t trial = 0; trial < num_trials; trial++) {
        ...
        matfunc.apply(d_result, d_input, dt / 2, MatrixFunctionApplicatorReal::FunctionType::SINC2_SQRT);
