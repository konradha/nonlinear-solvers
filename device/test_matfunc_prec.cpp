...
void test_solver_matfunc(const Eigen::SparseMatrix<double> &L,
                         const Eigen::VectorXd &u0, const double dt,
                         const uint32_t m = 10) {
  ...
  cudaMemcpy(d_row_ptr, L.outerIndexPtr(), (n + 1) * sizeof(int),
...

  {
    ...
    std::cout
        << "\nDiff norm: "
        << (Eigen::Map<Eigen::VectorXd>(result.data(), n) - host_result).norm()
        << "\n\n";
  }

  {
    auto host_result = sinc2_sqrt_half<double>(L, u0, dt, m);
    matfunc.apply(d_result, d_u, dt / 2,
