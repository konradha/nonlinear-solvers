...
__global__ void matrix_multiply_QDQ(const thrust::complex<double> *Q, const thrust::complex<double> *D,
...
public:
  ...
  void apply(thrust::complex<double> *result, const thrust::complex<double> *input,
    ...
    // thrust::apply
    transform_eigenvals_exp<<<grid_1d_, block_1d_>>>(
          d_diag_, d_eigenvalues_, dt, m_);
 
    matrix_multiply_QDQ<<<grid_QDQ_, block_2d_>>>(d_eigenvectors_, d_diag_,
                                                d_work_, m_);
    // gemmZ
    matrix_multiply_VK<<<grid_VK_, block_2d_>>>(krylov_.V, d_work_, d_work_large_,
                                              n_, m_);
    // dscalZ
