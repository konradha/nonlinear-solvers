...
public:
    ...
    MatrixFunctionApplicatorComplex(
        const int* d_row_ptr, 
        const int* d_col_ind,
        const cuDoubleComplex* d_values, 
        uint32_t n, 
        uint32_t m,
        uint32_t nnz,
        ...
        cudaMemcpy(thrust_values, d_values, nnz * sizeof(cuDoubleComplex), cudaMemcpyDeviceToDevice);
        
        spmv_ = new DeviceSpMV<thrust::complex<double>>(d_row_ptr, d_col_ind, thrust_values, n, nnz);
        ...
        cusolverDnZheevd_bufferSize(
            ...
            m,
            reinterpret_cast<cuDoubleComplex*>(d_eigenvectors_),
            m,
            d_eigenvalues_,
            &lwork);
    ...
    void apply(cuDoubleComplex* result, const cuDoubleComplex* input, thrust::complex<double> dt) {
      auto input_thrust = reinterpret_cast<const thrust::complex<double> *>(input);
      lanczos_iteration_complex(spmv_, &krylov_, input_thrust);
      double beta;
      cudaMemcpy(&beta, krylov_.reconstruct_beta, sizeof(double), cudaMemcpyDeviceToHost);
      compute_eigen_decomposition();


      Eigen::MatrixX<std::complex<double>> T = Eigen::MatrixX<std::complex<double>>::Zero(m_, m_);
      Eigen::MatrixX<std::complex<double>> V = Eigen::MatrixX<std::complex<double>>::Zero(n_, m_);

      cudaMemcpy(T.data(), krylov_.T, m_ * m_ * sizeof(std::complex<double>), cudaMemcpyDeviceToHost); 
      cudaMemcpy(V.data(), krylov_.V, n_ * m_ * sizeof(std::complex<double>), cudaMemcpyDeviceToHost);

      Eigen::MatrixX<std::complex<double>> evs = Eigen::MatrixX<std::complex<double>>::Zero(m_, m_); 
      Eigen::VectorX<std::complex<double>> lambdas = Eigen::VectorX<std::complex<double>>::Zero(T.rows()); 

      cudaMemcpy(evs.data(), d_eigenvectors_, m_ * m_ * sizeof(std::complex<double>), cudaMemcpyDeviceToHost); 
      cudaMemcpy(lambdas.data(), d_eigenvalues_, m_ * sizeof(std::complex<double>), cudaMemcpyDeviceToHost); 
      std::complex<double> tau(dt.real(), dt.imag()); 
      Eigen::MatrixX<std::complex<double>> exp_T = evs * (tau * lambdas.array().abs().unaryExpr([](std::complex<double> x){
		      return std::exp(x);})).matrix().asDiagonal()* evs.transpose(); 
      Eigen::VectorX<std::complex<double>> e1 = Eigen::VectorX<std::complex<double>>::Zero(T.rows());
      e1(0) = 1.0;
      e1 = beta * V * exp_T * e1; 
      cudaMemcpy(result, e1.data(), n_ * sizeof(std::complex<double>), cudaMemcpyHostToDevice);
...
private:
    void compute_eigen_decomposition() {
        ...
        cusolverDnZheevd(
            ...
            m_,
            d_eigenvalues_,
            reinterpret_cast<cuDoubleComplex*>(d_solver_work_),
            solver_work_size_,
            d_info_);
    }
