...
__global__ void check_nans_kernel(int* has_nan, const double* arr, const uint32_t n) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
      if (isnan(arr[idx])) atomicAdd(has_nan, 1);
  }
}

__global__ void neg_sin_kernel_2d(double* out, const double* in, const uint32_t nx, const uint32_t ny) {
...
__global__ void curr_velocity_2d(double* v, const double* u, const double * u_past, const double tau, const uint32_t nx, const uint32_t ny) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(x < nx && y < ny) {
        const int idx = y * nx + x;
...
__global__ void gautschi_kernel_2d(double* u_next, const double* u_past, 
                                 const double* costu, const double* filtered_sinc,
                                 const double tau, const uint32_t nx, const uint32_t ny) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(x < nx && y < ny) {
        const int idx = y * nx + x;
        u_next[idx] = 2.0 * costu[idx] - u_past[idx] + tau * tau * filtered_sinc[idx];
    }
}

__global__ void calculate_energy_kernel_2d(double* energy, const double* u, const double* v,
                                       const double* Lu, const double* c, const double* m,
    ...
    if(tid == 0) energy[blockIdx.y * gridDim.x + blockIdx.x] = sdata[0];
}

__global__ void validate_coverage_2d(bool* coverage_map, const uint32_t nx, const uint32_t ny) {
    const int x = blockIdx.x * blockDim.x + threadIdx.x;
    const int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(x < nx && y < ny) {
        const int idx = y * nx + x;
...
public:
    struct Parameters {
        ...
        Parameters(uint32_t ns = 100, uint32_t freq = 5, uint32_t m = 10) 
            : block_size(256), num_snapshots(ns), snapshot_freq(freq), krylov_dim(m) {}
    };

    SGESolverDevice(const int* d_row_ptr, const int* d_col_ind,
                    const double* d_values, const double* h_c, 
                    const double* h_m, uint32_t n, uint32_t nnz,
		    const double* host_u0, const double* host_v0, 
		    const double* host_u_past,
	...
	cudaMemcpy(d_v_, host_v0, n * sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(d_u_past_, host_u_past, n * sizeof(double), cudaMemcpyHostToDevice);
    ...
    void step(const double tau, const uint32_t step_number) {
      ...
      curr_velocity_2d<<<grid_dim_, block_dim_>>>(d_v_, d_u_, d_u_past_, tau, nx_, ny_);
    ...
    void transfer_snapshots(double * dst, char which='u') {
      if (which == 'u')
        cudaMemcpy(dst, d_u_trajectory_, params_.num_snapshots * n_ * sizeof(double), cudaMemcpyDeviceToHost);
      else if (which == 'v')
        cudaMemcpy(dst, d_v_trajectory_, params_.num_snapshots * n_ * sizeof(double), cudaMemcpyDeviceToHost);
      else if (which == 'e')
	cudaMemcpy(dst, d_e_trajectory_, params_.num_snapshots * sizeof(double), cudaMemcpyDeviceToHost);
      else
...
private:
    ...
    // u_{n+1} = 2 cos (tau \Omega) u_{n} - u_{n-1} + tau² sinc²(tau / 2 \Omega) x g(\phi(tau \Omegau_{n}))
    double* d_c_;
    double* d_m_;
    ...
    void verify_domain_coverage() {
      ...
      cudaMemcpy(h_coverage, d_coverage, n_ * sizeof(bool), cudaMemcpyDeviceToHost);
      ...
      if(complete_coverage) {
          printf("Complete domain coverage verified: all %u x %u points are processed\n", nx_, ny_);
      } else {
	  throw std::runtime_error("Kernels will not traverse entire grid correctly\n");
      }
