...
public:
    ...
    NLSESolverDevice(const Eigen::SparseMatrix<std::complex<double>> &L,
        ...
        cudaMalloc(&d_u_trajectory_, n_ * params_.num_snapshots * sizeof(thrust::complex<double>));
    ...
    void step(const std::complex<double> tau, const uint32_t step_number) {
        density<<<grid_dim_, block_dim_>>>(d_density_, d_u_, 
                                                         nx_, ny_); 
        nonlin_part<<<grid_dim_, block_dim_>>>(d_buf_, d_u_, 
                                                          d_density_,
                                                          -.5 * tau, nx_, ny_);
        matfunc_->apply(d_u_, d_buf_, -tau);
	
        density<<<grid_dim_, block_dim_>>>(d_density_, d_u_, nx_, ny_);
        nonlin_part<<<grid_dim_, block_dim_>>>(d_u_, d_u_, 
                                                          d_density_,
                                                          -.5 * tau, nx_, ny_);
        cudaError_t err = cudaGetLastError();
        if (err != cudaSuccess) {
