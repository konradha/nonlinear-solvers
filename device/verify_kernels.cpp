...
private:
   ...
   Eigen::VectorXd verify_transform_eigenvals(const Eigen::VectorXd& eigenvals) {
       return (t_ / 2. * eigenvals.array().abs().sqrt())
           .unaryExpr([](double x) { return std::abs(x) < 1e-8 ? 1.0 : std::sin(x) / x; })
           .square();
   }

   Eigen::MatrixXd verify_eigvec_transform(const Eigen::MatrixXd& Q, const Eigen::VectorXd& f_lambda) {
       return Q * f_lambda.asDiagonal() * Q.transpose();
   }

   Eigen::VectorXd verify_final_multiply(const Eigen::MatrixXd& V, const Eigen::MatrixXd& transform) {
...
public:
   ...
   void run_verification() {
       ...
       cudaMemcpy(d_eigvals, es.eigenvalues().data(), m_*sizeof(double), cudaMemcpyHostToDevice);
       cudaMemcpy(d_eigvecs, es.eigenvectors().data(), m_*m_*sizeof(double), cudaMemcpyHostToDevice);
       ...
       cudaMemcpy(cuda_temp.data(), d_temp, m_*sizeof(double), cudaMemcpyDeviceToHost);
       std::cout << "Stage 1 diff - L1: " << (eigen_temp - cuda_temp).lpNorm<1>() 
                << " L2: " << (eigen_temp - cuda_temp).lpNorm<2>() << std::endl;

       
       eigvec_transform<<<grid_2d_, block_2d_>>>(d_sinc_result, d_eigvecs, d_temp, m_);
       Eigen::MatrixXd eigen_sinc = verify_eigvec_transform(es.eigenvectors(), eigen_temp);
       Eigen::MatrixXd cuda_sinc(m_, m_);
       cudaMemcpy(cuda_sinc.data(), d_sinc_result, m_*m_*sizeof(double), cudaMemcpyDeviceToHost);
       std::cout << "Stage 2 diff - L1: " << (eigen_sinc - cuda_sinc).lpNorm<1>() 
                << " L2: " << (eigen_sinc - cuda_sinc).lpNorm<2>() << std::endl;

       final_multiply<<<grid_1d_, block_1d_>>>(d_final, d_V, d_sinc_result, 1.0, n_, m_);
       Eigen::VectorXd eigen_final = verify_final_multiply(V, eigen_sinc);
       Eigen::VectorXd cuda_final(n_);
       cudaMemcpy(cuda_final.data(), d_final, n_*sizeof(double), cudaMemcpyDeviceToHost);
