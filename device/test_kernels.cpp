...
void test_matrix_operations(uint32_t n, uint32_t m, uint32_t num_trials = 10) {
  ...
  for (uint32_t trial = 0; trial < num_trials; ++trial) {
    Eigen::MatrixXd V =
        Eigen::MatrixXd::NullaryExpr(n, m, [&]() { return dist(gen); });
    Eigen::MatrixXd K =
        Eigen::MatrixXd::NullaryExpr(m, m, [&]() { return dist(gen); });
    Eigen::MatrixXd Q =
        Eigen::MatrixXd::NullaryExpr(m, m, [&]() { return dist(gen); });
    Eigen::VectorXd D =
        Eigen::VectorXd::NullaryExpr(m, [&]() { return dist(gen); });
    ...
    std::cout << "Trial " << trial
              << " V x K error: " << (result_VK - ref_VK).norm() << "\n";
    ...
    std::cout << "Trial " << trial
              << " QDQ^T error: " << (result_QDQ - ref_QDQ).norm() << "\n";
    ...
    std::cout << "Trial " << trial
              << " beta * X[0,:] error: " << (result_scale - ref_scale).norm()
              << "\n\n";
  }
